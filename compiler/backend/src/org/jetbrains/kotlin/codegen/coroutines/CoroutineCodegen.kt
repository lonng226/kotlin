    ...
    override fun generateBody() {
        ...
        if (allLambdaParameters().size <= 1) {
            ...
            val bridgeParameters =
                    (1..delegate.argumentTypes.size - 1).map { AsmTypes.OBJECT_TYPE } +
                    delegate.argumentTypes.lastOrNull().singletonOrEmptyList()
    ...
    companion object {
        ...
        fun create(
            ...
            val invokeDescriptor =
                    expressionCodegen.state.bindingContext[CodegenBinding.SUSPEND_NAMED_FUNCTION_TO_INVOKE_DESCRIPTOR, originalSuspendDescriptor]!!

            val cv = state.factory.newVisitor(
                    OtherOrigin(declaration, originalSuspendDescriptor),
                    CodegenBinding.asmTypeForAnonymousClass(state.bindingContext, invokeDescriptor),
        ...
        private fun create(
                expressionCodegen: ExpressionCodegen,
                originalSuspendDescriptor: FunctionDescriptor?,
                invokeDescriptor: FunctionDescriptor,
                jvmViewOfInvokeDescriptor: FunctionDescriptor,
                declaration: KtFunction,
                classBuilder: ClassBuilder
        ) = CoroutineCodegen(
                ...
                expressionCodegen.context.intoCoroutineClosure(
                        jvmViewOfInvokeDescriptor, invokeDescriptor, expressionCodegen, expressionCodegen.state.typeMapper
                ),
                FunctionGenerationStrategy.FunctionDefault(expressionCodegen.state, declaration),
