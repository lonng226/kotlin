        ...
        if (context instanceof MethodContext) {
            ...
            if (isInterface(functionDescriptor.getContainingDeclaration()) && !isJvm8InterfaceMember(functionDescriptor, state)) {
                if (((MethodContext) context).isDefaultFunctionContext()) {
                    return typeMapper.mapDefaultMethod(functionDescriptor, OwnerKind.DEFAULT_IMPLS);
                }
                else {
                    return typeMapper.mapAsmMethod(functionDescriptor, OwnerKind.DEFAULT_IMPLS);
                }
            }

            return typeMapper.mapAsmMethod(functionDescriptor, OwnerKind.IMPLEMENTATION);
