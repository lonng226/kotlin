...
open class AbstractQuickFixMultiModuleTest : AbstractMultiModuleTest() {

    override val testPath: String = PluginTestCaseBase.getTestDataPathBase() + "/multiModuleQuickFix/"
    ...
    protected fun doTestWithoutExtraFile(beforeFileName: String) {
        doTest(beforeFileName, false)
    }

    @Throws(Exception::class)
    protected fun doTestWithExtraFile(beforeFileName: String) {
    ...
    private fun enableInspectionTools(klass: Class<*>) {
        ...
        val tool = eps.firstOrNull {
            klass.name == it.implementationClass
        }?.instantiateTool() ?: throw AssertionError("Could not find inspection tool for class: $klass")
    ...
    protected fun configureMultiFileTest(subFiles: List<TestFile>, beforeFile: TestFile) {
        try {
            ...
            val virtualFiles = java.util.HashMap<TestFile, VirtualFile>()
    ...
    protected fun doMultiFileTest(beforeFileName: String) {
        ...
        try {
            val subFiles = KotlinTestUtils.createTestFiles<Void, TestFile>(
                    "single.kt",
                    multifileText,
            ...
            CommandProcessor.getInstance().executeCommand(project, {
                try {
                    ...
                    val pair = LightQuickFixTestCase.parseActionHint(psiFile, beforeFile.content)
    ...
    private fun doTest(beforeFileName: String, withExtraFile: Boolean) {
        ...
        try {
            if (withExtraFile) {
                ...
                val extraFiles = mainFileDir.listFiles(
                        { _, name -> name.startsWith(extraFileNamePrefix(mainFileName)) && name != mainFileName }
                )!!

                val testFiles = java.util.ArrayList<String>()
            ...
            CommandProcessor.getInstance().executeCommand(project, {
                try {
                    ...
                    val pair = LightQuickFixTestCase.parseActionHint(psiFile, originalFileText)
                    val text = pair.getFirst()
    ...
    protected class TestFile internal constructor(val path: String, val content: String)
}
