...
class CoroutineFunctionTransformer(private val program: JsProgram, private val function: JsFunction) {
    ...
    private fun generateCoroutineMetadata(constructorName: JsName): JsStatement {
        ...
        metadataObject.propertyInitializers += JsPropertyInitializer(
                JsNameRef(Namer.METADATA_CLASS_KIND), JsNameRef(Namer.CLASS_KIND_CLASS, JsNameRef(Namer.CLASS_KIND_ENUM, Namer.KOTLIN_NAME)))
        metadataObject.propertyInitializers += JsPropertyInitializer(JsNameRef(Namer.METADATA_SIMPLE_NAME), JsLiteral.NULL)
        metadataObject.propertyInitializers += JsPropertyInitializer(JsNameRef(Namer.METADATA_SUPERTYPES), JsArrayLiteral(listOf(baseClassRefRef)))
